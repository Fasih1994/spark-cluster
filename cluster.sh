...
function startServices {
  ...
  docker exec -u hadoop -d nodemaster hdfs dfs -mkdir /spark-jars
  docker exec -u hadoop -d nodemaster hdfs dfs -mkdir /spark-logs
  sleep 5
...
if [[ $1 = "install" ]]; then
  ...
  docker run -d --net hadoopnet --ip 172.20.1.1 -p 8088:8088 -p 4040:4040 -p 18080:18080 -p 8080:8080 --hostname nodemaster --add-host worker1:172.20.1.2 --add-host worker2:172.20.1.3 --name nodemaster -it fashi/spark:3.3.1
...
if [[ $1 = "uninstall" ]]; then
  ...
  docker rm nodemaster worker1 worker2
  exit
fi
